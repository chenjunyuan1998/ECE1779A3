...
def put_stats(num_items, size_items, num_requests, db_num_hit, db_num_miss):
    ...
    if rowcount == 0:  # table `ImageDB`.`Stats` is empty, this should never happen
    ...
    else:  # table `ImageDB`.`Stats` is not empty
        query = "UPDATE `ImageDB`.`Stats` SET `num_items` = {}, `size_items` = {}, `num_requests` = {}, " \
                + "`num_hit` = {}, `num_miss` = {}, `_timestamp` = CURRENT_TIMESTAMP;"
        cursor.execute(query.format(num_items, size_items, num_requests, db_num_hit, db_num_miss))
...
#     initial_stat = get_stats()
#     print("initial get_stats() result: ", initial_stat)
#
#     put_stats(1, 1, 1, 1, 1)
#     latest_stat = get_stats()
#     print("latest get_stats() result: ", latest_stat)
#
#     # Since there is only one row left in the table, this test function is no longer useful
#     # clear_stats()
#     # stat = get_stats()
